...
resource "aws_iam_policy" "autoscaler" {
  name = "shriganesh-eks-autoscaler-policy1"
  policy = jsonencode({
    Version = "2012-10-17",
    Statement = [{
      Action = [
...
resource "aws_iam_instance_profile" "worker" {
  ...
  name       = "shriganesh-eks-worker-profile1"
  role       = aws_iam_role.worker.name
}
...
resource "aws_eks_node_group" "node-grp" {
  ...
  tags = {
    Name = "shriganesh-eks-node-group"
  }
